...
We also suggest to install currently newest version RStudio Version (Version 1.4 or later), which offers a visual RMarkdown/Rnotebooks editor.
...
The data used in this workshop is from \*Chew et al., 2019, JCI Insight 5(13)\* [10.1172/jci.insight.126925](https://doi.org/10.1172/jci.insight.126925). The lipdodmics LC-MS raw data used in this publication was reprocessed using MRMkit \*Teo et al., 2020, Analytical Chemistry 92, 20\* [10.1021/acs.analchem.0c03060](https://doi.org/10.1021/acs.analchem.0c03060).
...
This dataset contains also 2 QC types: BQCs (BatchQC, pooled plasma co-extracted and measured with samples) and TQCs (technical QC, pooled extract measured in regular intervals). We now calculate the %CV (coefficient of variation) of all samples for each species. This can be done in the present format of the data, another way is to convert the table first into a 'long' format:
...
Calculate the CVs and plot histogram. Reshaping the table so that the CVs for BQC, TQC and samples are columns will make it easier read.
...
## Plotting the run order vs concentration

```{r fig.height=10, fig.width=10}

# we will use run_id column that was generated

d_plt <- d_long %>% filter(lipid %in% c("Cer d18:1/C16:0", "Cer d18:1/C18:0", "Cer d18:1/C24:0", "Cer d18:1/C24:1" ))

# the layer of the QCtype points is defined by the order of the levels in the factor, so lets re-arrange them to
# move BQC on top
...
d_plt <- d_plt %>% arrange(type)
 ...
 p <- ggplot(d_plt, aes(x=run_id, y=conc)) + 
        geom_point(aes(colour = type, fill = type, shape  = type), size= 0.4, alpha=0.5, stroke = 1) +
...
# ensure folder "Part_1/output" exists
ggsave(plot = p, filename = here("Part_1/output", "run_scatter_plot.pdf") )

```

## Converting lipid species names to the updated LIPID MAPS nomenclature and retrieving lipid annotations

LIPID MAPS has recently updated the lipid species nomenclature to a more comprehensive and standardized system [Liebisch et al, Lipid Res, 2020](https://doi.org/10.1194/jlr.s120001025). There is an increasing number of publications adopting this new nomenclature. There is an online service to convert lipid names (<https://apps.lifs.isas.de/goslin/>). The software behind this tool is also available in R ([rgoslin](https://github.com/lifs-tools/rgoslin)) and Python ([pygoslin](https://github.com/lifs-tools/pygoslin)).
...
#### Prepare the data and get p-values and log2(fold changes) M vs F for each lipid species (method 1)
...
#### Now using packages `purrr` and `broom` to do the get comprehensive t.test results for all lipid species
...
Each row in 'data' column contains a table with the data for one species
...
-   [Cheat Sheats for Tidyverse packages](https://rstudio.com/resources/cheatsheets)
